...
# predict error duration > 10*60 sek
...
from mikkelLoadData import dataSek

style.use('default') # Set plot theme
np.random.seed(2)


dataSek = dataSek.drop(['Feed'],axis=1)
dataSek = dataSek.drop(['OutputGood'],axis=1)
dataSek = dataSek.drop(['Error_Count'],axis=1)
...
#X = stats.zscore(X)
y = np.zeros(np.shape(mat_data)[0])

for i in range(np.shape(mat_data)[0]):
    if X[i,2] > 500:
        y[i]=1
X = np.delete(data,[2,],1).squeeze()


classNames = [ 'fine','bad']
dataSek = dataSek.drop(['Sum_Duration'],axis=1) 

attributeNames = list(dataSek)
...
tree.export_graphviz(dtc_fit, out_file='tree_gini2.gvz', feature_names=attributeNames)
